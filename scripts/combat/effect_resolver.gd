...
func _apply_effect(
    ...
    match op_type:
        ...
        "consume_mark_to_damage":
            ...
            var consumed := min(mark_cap, state.enemy.consume_all_marks())
            if consumed > 0:
        ...
        "damage_self":
            var self_damage := max(0, int(effect.get("value", "0")))
...
func _do_damage(
    ...
    if unit_id == "helios_windchaser" and (face.face_id == "helios_sniper" or face.face_id == "helios_pierce"):
        var focus_used := min(3, state.player.focus_stacks)
        if focus_used > 0:
    ...
    if unit_id == "umbral_draxx" and (face.face_id == "black_execute" or face.face_id == "black_reap"):
        execute_bonus = int((state.enemy.max_hp - state.enemy.hp) / 4)
...
func _mod_resource(state: CombatState, spec: String, face: DiceFaceDefinition, logger: ActionLogger) -> void:
...
func _log_simple(
    ...
    out["face_id"] = face_id
    logger.append(ActionLogEntry.new(event_type, state.turn_index, actor_id, target_id, out))
